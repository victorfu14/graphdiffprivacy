        ...
        for idx, lines in enumerate(self.sequence):
            ...
            for (key, value) in self.options.items():
                if value == True:
                    ...
                    # logger.info('Batch: {}'.format(idx))
                    # logger.info('{}:'.format(key))
                    # logger.info('Real statistic: {}'.format(self.stat_data[key][idx]))
                    # logger.info('Noisy statistic: {}'.format(self.stat_noisy[key][idx]))
    ...
    def plot(self, plot=None, option: Literal['value', 'error'] = 'error', out_dir=None):
        ...
        for (key, value) in self.options.items():
            if value == True:
                ...
                ax.plot(
                    np.array(self.max_degree_list) / self.epsilon * math.log(self.timesteps) * math.log(1 / 0.1), 
                    linewidth=2.0, 
                    label='theoretical_error_T',
                    color='red'
                )
                ax.axhline(2 * self.max_degree / self.epsilon * math.sqrt(self.psum_size) * math.log(1 / 0.1), color='orange')
                ax.axhline(self.max_degree / self.epsilon * math.log(self.timesteps) * math.log(1 / 0.1), color='red')
